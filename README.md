# Aoyu Liu — Personal Page

**本站已改为单页：** 直接使用根目录的 **`index.html`**，包含 About Me、News、Publications、Co-authors 四个区块，顶部导航点击可跳转到对应位置。

- 若使用 **GitHub Pages**：在仓库 **Settings → Pages** 里选择 **“Deploy from a branch”**，分支选 `main`（或你的默认分支），Source 选 **“/ (root)”**，这样会直接提供静态 `index.html`，无需 Jekyll。
- 论文 PDF/BibTeX 等请放在 `files/` 目录，图片可放在 `images/` 目录。
